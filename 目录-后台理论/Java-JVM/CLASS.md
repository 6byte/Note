...
```
1. .CLASS文件只要符合JVM规范,可以由其他语言，或不用语言也能生成
2.	.class实质：是编译产生的二进制文件，严格按照JVM的规范。以表的形式组织代码中的各个部分,.class=一张表
3.  .class表重要组成部分:常量池，访问标记，当前类，类的属性，类的方法，类的字段，类的接口，父类，magic!
...
2.对于引用类型的静态变量,未用new为其分配对象（如：static Object obj;）那么对象的引用obj会存储在方法区中，并为其指定默认值null;

3.对于引用类型的静态变量，用new关键字为其分配对象（如：static Person person = new Person();）,那么对象的引用person 会存储在方法区中，并且该对象在堆中的地址也会存储在方法区中（注意此时静态变量只存储了对象的堆地址，而对象本身仍在堆内存中）
4.字节码文件，静态方法，

栈<虚拟机栈>
八种基本数据类型，局部变量，局部的对象的引用
